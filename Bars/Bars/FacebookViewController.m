...
- (id)init {
    ...
    if (self) {
        ...
        BarsAppDelegate *delegate = (BarsAppDelegate *)[[UIApplication sharedApplication] delegate];
...
- (void) apiGraphUserPhotosPost {
    ...
    _fbRequest = FBUpload;
    NSString *tag = [NSString stringWithFormat:@"%@ at %@, %@",_photoTag.text, _bar.barName, _bar.barAddress];
    NSString * location = [_bar barLocation];
    NSArray *arr = [location componentsSeparatedByString:@"~"];
    NSString *locationTag = [NSString stringWithFormat:@"%@,%@", [arr objectAtIndex:0],[arr objectAtIndex:1]];
