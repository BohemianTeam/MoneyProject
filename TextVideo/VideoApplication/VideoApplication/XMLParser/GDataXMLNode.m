  ...
  if (xmlNode_ != NULL) {
    ...
    if (attrPtr) {
      ...
      if (oldAttr == NULL) {
        ...
        // Added these lines since we detected a leak for variable value in Intstruments
        if (value)
              xmlFree(value);
...
+ (void)fixQualifiedNamesForNode:(xmlNodePtr)nodeToFix
  ...
  if (nodeToFix->ns == NULL) {
    ...
    if (localName != NULL) {
      ...
      // Added this because of memory leak detected in Intreruments for variable loalname  
      if (xmlStrcmp(nodeToFix->name,localName))
            xmlFree(localName);
