...
- (void)processError:(NSError *)error {
    @try {
        ...
        if (self.canShowAlert) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:LocStr(@"AlertTitleNotConnect")
                                                            message:LocStr(@"AlertMsgNotConnect") 
                                                           delegate:nil 
...
- (void)processData:(NSData *)data {
    ...
    NSString *content = [NSString stringWithUTF8String:[buff bytes]];
    NSLog(content);
    @try {
        [self reset];
        switch (act) {
...
+ (void)showErrorNetwork {
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:LocStr(@"AlertTitleErrorNetwork") 
