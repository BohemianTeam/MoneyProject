...
public class Workspace extends ViewGroup {
	...
	/**
	 * The user needs to drag at least this much for it to be considered a fling
	 * gesture. This reduces the chance of a random twitch sending the user to
	 * the next screen.
	 */
	// TODO: refactor
	...
	/**
	 * This view's X scroll offset when the active pointer was first pressed
	 * down.
	 */
	 ...
	 * @param attrs
	 *            The attributes set containing the
	 *            com.google.android.ext.workspace.Workspace's customization
	 *            values.
	 */
	...
	void notifyScreenChangeListener(int whichScreen, boolean changeComplete) {
		if (mOnScreenChangeListener != null) {
			if (changeComplete)
				mOnScreenChangeListener.onScreenChanged(
						getScreenAt(whichScreen), whichScreen);
			else
				mOnScreenChangeListener.onScreenChanging(
						getScreenAt(whichScreen), whichScreen);
		}
	...
	public void computeScroll() {
		if (mScroller.computeScrollOffset()) {
			...
			handleScreenChangeCompletion(Math.max(0,
					Math.min(mNextScreen, getScreenCount() - 1)));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < count; i++) {
			if (mSeparatorDrawable != null && (i & 1) == 1) {
				// separator
				getChildAt(i).measure(mSeparatorDrawable.getIntrinsicWidth(),
						heightMeasureSpec);
			} else {
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (mDeferredScreenChange >= 0) {
			snapToScreen(mDeferredScreenChange, mDeferredScreenChangeFast,
					mDeferredNotify);
	...
	public boolean requestChildRectangleOnScreen(View child, Rect rectangle,
		...
		if (mIgnoreChildFocusRequests && !mScroller.isFinished()) {
			Log.w(TAG, "Ignoring child focus request: request "
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (mIsVerbose) {
			Log.v(TAG, "onInterceptTouchEvent: "
					+ (ev.getAction() & MotionEventUtils.ACTION_MASK));
		}
		...
		case MotionEvent.ACTION_MOVE: {
			...
			/*
			 * Locally do absolute value. mDownMotionX is set to the y value of
			 * the down event.
			 */
			final int pointerIndex = MotionEventUtils.findPointerIndex(ev,
					mActivePointerId);
		...
		if (mIsVerbose) {
			Log.v(TAG,
					"Intercepting touch events: " + Boolean.toString(intercept));
		}
		return intercept;
	}

	void onSecondaryPointerUp(MotionEvent ev) {
		final int pointerIndex = (ev.getAction() & MotionEventUtils.ACTION_POINTER_INDEX_MASK) >> MotionEventUtils.ACTION_POINTER_INDEX_SHIFT;
		...
		if (pointerId == mActivePointerId) {
			...
			mDownScrollX = getScrollX();
			mActivePointerId = MotionEventUtils.getPointerId(ev,
					newPointerIndex);
			if (mVelocityTracker != null) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				scrollTo(
						Math.max(
								0,
								Math.min(maxScrollX, (int) (mDownScrollX
										+ mDownMotionX - x))), 0);
				if (mOnScrollListener != null) {
					mOnScrollListener.onScroll(getCurrentScreenFraction());
				}
		...
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				// TODO(minsdk8): int velocityX = (int)
				// MotionEventUtils.getXVelocity(velocityTracker,
				// activePointerId);
	...
	/**
	 * Returns the current scroll position as a float value from 0 to the number
	 * of screens. Fractional values indicate that the user is mid-scroll or
	 * mid-fling, and whole values indicate that the Workspace is currently
	 * snapped to a screen.
	 */
	...
	void snapToScreen(int whichScreen, boolean fast, boolean notify) {
		...
		final boolean screenChanging = (mNextScreen != INVALID_SCREEN && mNextScreen != whichScreen)
		...
		if (focusedChild != null && screenDelta != 0
				&& focusedChild == getScreenAt(mCurrentScreen)) {
			// clearing the focus of the child will cause focus to jump to the
			// tabs,
			// which will in turn cause snapToScreen to be called again with a
			// different
			// value. To prevent this, we temporarily disable the
			// OnTabClickListener
			// if (mTabRow != null) {
		...
		if (mNextScreen != mCurrentScreen) {
			...
			if (screenAt != null) {
				...
				Log.e(TAG, "Screen at index was null. mCurrentScreen = "
						+ mCurrentScreen);
				return;
			}

			// showing the filter popup for the next listview needs to be
			// delayed
	...
	/**
	 * Register a callback to be invoked when the screen is changed, either
	 * programmatically or via user interaction. Will automatically trigger a
	 * callback.
	 * 
	 ...
	 *            Whether to trigger a notification immediately
	 */
	public void setOnScreenChangeListener(
			OnScreenChangeListener screenChangeListener,
		...
		if (mOnScreenChangeListener != null && notifyImmediately) {
			mOnScreenChangeListener.onScreenChanged(
					getScreenAt(mCurrentScreen), mCurrentScreen);
		}
	}

	/**
	 * Register a callback to be invoked when this Workspace is mid-scroll or
	 * mid-fling, either due to user interaction or programmatic changes in the
	...
	/**
	 * Scrolls to the given screen fast (no matter how large the scroll distance
	 * is)
	 * 
	 * @param screenIndex
	 */
	...
	/**
	 * Scrolls to the screen adjacent to the current screen on the left, if it
	 * exists. This method is a no-op if the Workspace is currently locked.
	 */
	...
	/**
	 * Scrolls to the screen adjacent to the current screen on the right, if it
	 * exists. This method is a no-op if the Workspace is currently locked.
	 */
	...
	/**
	 * Locks the current screen, preventing users from changing screens by
	 * swiping.
	 */
	 ...
	 * {@link Workspace#lockCurrentScreen()}.
	 */
	...
	/**
	 * Sets the resource ID of the separator drawable to use between adjacent
	 * screens.
	 */
	...
	public void addViewToFront(View v) {
		mCurrentScreen++;
		addView(v, 0);
		snapToScreen(mCurrentScreen, true, false);
	}
